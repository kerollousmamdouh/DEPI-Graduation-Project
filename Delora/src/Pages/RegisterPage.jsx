...
const RegisterPage = () => {
  ...
  const sendOtpToBackend = async () => {
    try {
      ...
      console.log("استجابة التحقق من الحساب وإرسال الـ OTP:", response);

      setToast({ show: true, message: "تم التحقق من وجود الحساب وإرسال كود التحقق بنجاح! 📧" });
      setStep(2); 
    } catch (err) {
      console.error("خطأ أثناء التحقق من الحساب:", err);
      const serverMessage = err.message || "هذا الإيميل غير مسجل في جوجل أو غير حقيقي، يرجى كتابة بريد إلكتروني صالح!";
  ...
  const verifyOtpWithBackend = async () => {
    try {
      ...
      console.log("تم تفعيل الحساب وحفظ البيانات بنجاح:", response);

      setToast({ show: true, message: "تم إنشاء الحساب بنجاح! يمكنك تسجيل الدخول الآن 🎉" });
      navigate("/login");
    } catch (err) {
      console.error("خطأ أثناء تأكيد الحساب:", err);
      const serverMessage = err.message || "كود التحقق غير صحيح أو منتهي الصلاحية! ❌";
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setToast({ show: true, message: "تم إعادة إرسال كود التحقق بنجاح! 📧" });
      setResendCountdown(60);
    } catch (err) {
      setErrors((prev) => ({ ...prev, otp: err.message || "فشل إعادة إرسال الكود." }));
    } finally {
              ...
              <button
                ...
                className={`w-full text-center text-xs font-bold py-2 rounded-xl transition-colors ${
                  resendCountdown > 0
                    ...
                    : "text-green-400 lg:text-green-600 hover:bg-green-50 cursor-pointer"
                }`}
              >
                {resendLoading
                  ? "جاري الإرسال..."
