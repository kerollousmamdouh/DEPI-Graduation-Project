...
const CheckoutPage = ({ cartItems = [], onCheckout }) => {
  const { adminData, currentUser, paymentMethods } = useContext(SiteContext);
  ...
  const activePaymentMethods = Array.isArray(paymentMethods) && paymentMethods.length > 0
    ? paymentMethods
    : Array.isArray(adminData?.paymentMethods)
      ? adminData.paymentMethods
      : [];

  const selectedMethod = activePaymentMethods.find(
    (m) => String(m.id) === String(formData.payment) || m.name === formData.payment
  );

  const isDigitalMethod = selectedMethod && selectedMethod.type === "offline" && selectedMethod.receiverNumber;
  ...
  const handleSubmit = (e) => {
    ...
    if (selectedMethod?.requireScreenshot && !formData.image) { 
      setError("يرجى رفع صورة الإيصال لتأكيد عملية الدفع"); 
      return; 
    }
    if (selectedMethod?.requireTransactionId && !/^\d+$/.test(formData.transactionId)) { 
      setError("رقم المرجع يجب أن يكون أرقاماً فقط"); 
      return; 
    }
    ...
    const orderPayload = {
      ...
      paymentMethod: selectedMethod?.name || formData.payment,
      paymentDetails: (formData.transactionId || formData.image) ? {
        transactionId: formData.transactionId,
        receiptImage: formData.image
      } : null,
      status: "PENDING",
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-md bg-[#111] p-8 rounded-[2.5rem] border border-white/10 shadow-2xl">
        ...
        <div className="my-8">
          ...
          {activePaymentMethods.length > 0 ? (
            <div className="grid grid-cols-2 gap-3">
              {activePaymentMethods.map((method) => (
                <button 
                  type="button" 
                  key={method.id} 
                  onClick={() => setFormData({...formData, payment: String(method.id), transactionId: "", image: null})}
                  className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center justify-center ${
                    String(formData.payment) === String(method.id) 
                ...
                >
                  {method.logo ? (
                    <img src={method.logo} alt={method.name} className="w-8 h-8 object-contain mb-2" />
                  ) : (
            ...
            <div className="text-center text-slate-500 text-xs py-4">لا توجد طرق دفع متاحة حالياً</div>
          )}
        </div>

        {selectedMethod && selectedMethod.type === "offline" && selectedMethod.receiverNumber && (
          <div className="mb-6 p-5 bg-green-900/10 border border-green-500/30 rounded-2xl text-center relative animate-fade-in">
            <p className="text-green-400 text-xs font-bold mb-2">
              {selectedMethod.receiverName ? `${selectedMethod.receiverName} - ` : ""}رقم المحفظة (التحويل هنا):
            </p>
            <button type="button" onClick={() => copyToClipboard(selectedMethod.receiverNumber)} className="text-white text-xl font-black flex items-center justify-center gap-2">
              {selectedMethod.receiverNumber} <Copy size={16} className="text-slate-500" />
            </button>
        ...
        {selectedMethod && selectedMethod.type === "offline" && !selectedMethod.receiverNumber && (
          <div className="mb-6 p-4 bg-yellow-900/10 border border-yellow-500/30 rounded-2xl text-center text-yellow-500 text-xs font-bold">
            يرجى التواصل معنا للحصول على بيانات التحويل الخاصة بـ {selectedMethod.name}
          </div>
        )}

        {selectedMethod && (selectedMethod.requireScreenshot || selectedMethod.requireTransactionId) && (
          <div className="bg-linear-to-br from-green-900/20 to-[#111] p-6 rounded-2xl border border-green-500/30 mb-6 space-y-4">
            {selectedMethod.requireScreenshot && (
              <>
            ...
            {selectedMethod.requireTransactionId && (
              <div className="relative">
