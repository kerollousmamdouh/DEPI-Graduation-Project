...
function SmartProductCard({
  ...
  const isWeightType = category?.type === "weight";
   const isRtl = lang === 'ar';
  ...
  // استبدل دالة processDispatchToCart الحالية بهذا الكود
const processDispatchToCart = (requestedUnits, displayNote, gms = 0) => {
  const offerUnitPrice = Number(product.offerPrice);
  const regularUnitPrice = Number(product.price);

  if (hasOffer && requestedUnits > 0) {
    const offerPart = Math.min(requestedUnits, remainingOfferStock);
    const regularPart = requestedUnits - offerPart;

    if (offerPart > 0) {
      onAddToCart({
        ...product,
        price: isWeightType ? (offerUnitPrice * offerPart) / 1000 : offerUnitPrice * offerPart,
        quantity: isWeightType ? 1 : offerPart,
        weightGrams: isWeightType ? offerPart : 0,
        isOfferItem: true
      });
    }
    if (regularPart > 0) {
      onAddToCart({
        ...product,
        price: isWeightType ? (regularUnitPrice * regularPart) / 1000 : regularUnitPrice * regularPart,
        quantity: isWeightType ? 1 : regularPart,
        weightGrams: isWeightType ? regularPart : 0,
        isOfferItem: false
      });
    }
  } else {
    onAddToCart({
      ...product,
      price: isWeightType ? (regularUnitPrice * gms) / 1000 : regularUnitPrice * requestedUnits,
      quantity: isWeightType ? 1 : requestedUnits,
      weightGrams: isWeightType ? gms : 0,
      isOfferItem: false
    });
  }
  ...
  const handleAddToCart = (note) => {
   try {
    // 1. رسالة الخطأ عند عدم تحديد وزن أو سعر
    ...
    if (isWeightType) {
      if (note) {
        ...
        displayNote = lang === 'ar' ? `${gms} جرام` : `${gms} grams`;
      } else if (priceInput) {
        const targetPrice = parseFloat(priceInput);
        const unitPrice = hasOffer ? Number(product.offerPrice) : Number(product.price);
        gms = (targetPrice / unitPrice) * 1000;
        displayNote = lang === 'ar' ? `${priceInput} جنيه` : `${priceInput} EGP`;
      }
      reqUnits = gms;
    }

    // 2. رسالة خطأ تجاوز المخزون
    ...
    const amount = isWeightType 
        ? (weightInput ? parseFloat(weightInput) : (priceInput ? (parseFloat(priceInput) / (hasOffer ? Number(product.offerPrice) : Number(product.price))) * 1000 : 0)) 
        : quantity;
    ...
    onAddToCart({ ...product, isWeightType: isWeightType }, amount);
    setError("");
    setIsModalOpen(false);
   
    processDispatchToCart(reqUnits, displayNote, gms);
   console.log("محاولة إضافة للمنتج...");
  }
    catch (error) {
      ...
      <form onSubmit={handleFormSubmit} className="p-5 sm:p-6">
        ...
        {isWeightType ? (
          <div className="space-y-4">
            ...
            <input type="text" placeholder={lang === 'ar' ? "اكتب الوزن بالجرام..." : "Enter weight in grams..."} value={weightInput} onFocus={(e) => e.target.select()} onClick={(e) => e.target.select()} onChange={(e) => handleWeightChange(e.target.value)} className="w-full border border-gray-400 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-center text-xs sm:text-sm font-semibold outline-none focus:border-[#00a650]" />
            <input type="text" placeholder={lang === 'ar' ? "اطلب بمبلغ (بـ 50 جنيه)" : "Order by amount (e.g. 50 EGP)"} value={priceInput} onFocus={(e) => e.target.select()} onClick={(e) => e.target.select()} onChange={(e) => handlePriceChange(e.target.value)} className="w-full border border-gray-400 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-center text-xs sm:text-sm font-semibold outline-none focus:border-[#00a650]" />
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center gap-2 my-6 sm:my-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                className="w-20 h-12 border border-gray-400 rounded-xl text-center text-xl font-black text-gray-900 outline-none focus:border-[#00a650] shadow-inner"
              />
