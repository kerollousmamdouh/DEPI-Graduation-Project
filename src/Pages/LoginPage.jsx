...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      /* ==================================================================
      🟢 بلوك جاهز للربط مع الـ Backend (افتح الكومنت ده لما تجهز الـ API)
      ==================================================================
      const response = await fetch("https://api.dealoramarket.com/api/auth/login", {
        method: "POST",
      ...
      localStorage.setItem("dealora_market_current_user", JSON.stringify(data.user));
      if (loginUser) loginUser(data.user, data.token);
      ...
      */
      ...
      const userMatch = savedUsers.find(
        (u) => (u.email === formData.username || u.phone === formData.username) && u.password === formData.password
      );

      if (formData.username === "admin" && formData.password === "admin123") {
        const adminPayload = { fullName: "مدير النظام", email: "admin@dealora.com", role: "admin" };
        localStorage.setItem("dealora_market_token", "admin-mock-token");
        localStorage.setItem("dealora_market_current_user", JSON.stringify(adminPayload));
        if (loginUser) loginUser(adminPayload, "admin-mock-token");
        navigate("/dashboard");
        return;
      }

      if (userMatch) {
        localStorage.setItem("dealora_market_token", "user-mock-token");
        localStorage.setItem("dealora_market_current_user", JSON.stringify(userMatch));
        if (loginUser) loginUser(userMatch, "user-mock-token");
        navigate("/home");
      } else {
        setAuthError("اسم المستخدم أو كلمة المرور غير صحيحة / Invalid credentials");
      }

    } catch (error) {
      setAuthError(error.message || "حدث خطأ في الاتصال بالسيرفر / Server connection error");
    } finally {
      setIsLoading(false);
    }
  };
